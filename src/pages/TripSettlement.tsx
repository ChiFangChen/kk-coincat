...
export function TripSettlement({ trip, members }: Props) {
  ...
  const currencyBreakdown = calculateCurrencyBreakdown(
    expenses,
    trip.members,
    trip.primaryCurrency,
  );
            ...
            return (
              <div key={m.id} className="balance-item">
                ...
                <div className="balance-right">
                  <div
                    className={`balance-amount ${balance > threshold ? "positive" : balance < -threshold ? "negative" : ""}`}
                  >
                    {balance > threshold
                      ? `+${balance.toFixed(0)}`
                      : balance < -threshold
                  ...
                  {(() => {
                    ...
                    const hasNonPrimary = currencies.some(
                      (c) => c !== trip.primaryCurrency,
                    );
              ...
              <div key={i} className="transfer-item">
                <div className="transfer-info">
                  <span className="transfer-name">
                    ...
                    <span
                      className="color-dot"
                      style={{ backgroundColor: getUserColor(t.from) }}
                    />
                  </span>
                  <FontAwesomeIcon
                    icon={faArrowRight}
                  ...
                  <span className="transfer-name">
                    ...
                    <span
                      className="color-dot"
                      style={{ backgroundColor: getUserColor(t.to) }}
                    />
                  </span>
      ...
      {showCustomSettle && customSettleData && (
        <ExpenseForm
          ...
          editingExpense={{
            ...
            splitDetails: [
              { userId: customSettleData.to, value: customSettleData.amount },
            ],
            isSettlement: true,
            createdAt: new Date().toISOString(),
